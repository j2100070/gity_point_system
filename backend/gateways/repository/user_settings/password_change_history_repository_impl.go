...
func (r *PasswordChangeHistoryRepositoryImpl) Create(history *entities.PasswordChangeHistory) error {
	r.logger.Debug("Creating password change history", entities.NewField("user_id", history.UserID))
	return r.passwordChangeHistoryDS.Insert(history)
}

// ReadListByUserID はユーザーIDで履歴を取得
func (r *PasswordChangeHistoryRepositoryImpl) ReadListByUserID(userID uuid.UUID, offset, limit int) ([]*entities.PasswordChangeHistory, error) {
	return r.passwordChangeHistoryDS.SelectListByUserID(userID, offset, limit)
}

// CountByUserID はユーザーIDで履歴数を取得
func (r *PasswordChangeHistoryRepositoryImpl) CountByUserID(userID uuid.UUID) (int64, error) {
	return r.passwordChangeHistoryDS.CountByUserID(userID)
}
