...
func (r *ProductRepositoryImpl) Create(product *entities.Product) error {
	...
	return r.productDS.Insert(product)
}

// Read はIDで商品を検索
func (r *ProductRepositoryImpl) Read(id uuid.UUID) (*entities.Product, error) {
	return r.productDS.Select(id)
}
...
func (r *ProductRepositoryImpl) Update(product *entities.Product) error {
	...
	return r.productDS.Update(product)
}

// Delete は商品を論理削除
func (r *ProductRepositoryImpl) Delete(id uuid.UUID) error {
	r.logger.Debug("Deleting product", entities.NewField("product_id", id))
	return r.productDS.Delete(id)
}

// ReadList は商品一覧を取得
func (r *ProductRepositoryImpl) ReadList(offset, limit int) ([]*entities.Product, error) {
	return r.productDS.SelectList(offset, limit)
}
...
func (r *ProductRepositoryImpl) ReadAvailableList(offset, limit int) ([]*entities.Product, error) {
	return r.productDS.SelectAvailableList(offset, limit)
}

// Count は商品総数を取得
func (r *ProductRepositoryImpl) Count() (int64, error) {
	return r.productDS.Count()
}

// UpdateStock は在庫を更新
func (r *ProductRepositoryImpl) UpdateStock(tx interface{}, productID uuid.UUID, quantity int) error {
	return r.productDS.UpdateStock(tx, productID, quantity)
}
