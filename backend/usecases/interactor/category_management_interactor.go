...
func (i *CategoryManagementInteractor) CreateCategory(req *inputport.CreateCategoryRequest) (*inputport.CreateCategoryResponse, error) {
	i.logger.Info("Creating new category", entities.NewField("name", req.Name), entities.NewField("code", req.Code))

	// コードの重複チェック
	exists, err := i.categoryRepo.ExistsCode(req.Code, nil)
	if err != nil {
	...
	if err := i.categoryRepo.Create(category); err != nil {
...
func (i *CategoryManagementInteractor) UpdateCategory(req *inputport.UpdateCategoryRequest) (*inputport.UpdateCategoryResponse, error) {
	i.logger.Info("Updating category", entities.NewField("category_id", req.CategoryID))

	category, err := i.categoryRepo.Read(req.CategoryID)
	if err != nil {
...
func (i *CategoryManagementInteractor) DeleteCategory(req *inputport.DeleteCategoryRequest) error {
	i.logger.Info("Deleting category", entities.NewField("category_id", req.CategoryID))

	// カテゴリの存在確認
	_, err := i.categoryRepo.Read(req.CategoryID)
	if err != nil {
	...
	if err := i.categoryRepo.Delete(req.CategoryID); err != nil {
...
func (i *CategoryManagementInteractor) GetCategoryList(req *inputport.GetCategoryListRequest) (*inputport.GetCategoryListResponse, error) {
	categories, err := i.categoryRepo.ReadList(req.ActiveOnly)
	if err != nil {
		return nil, fmt.Errorf("failed to get category list: %w", err)
	}

	total, err := i.categoryRepo.Count()
	if err != nil {
